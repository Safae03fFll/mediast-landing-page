            ...
            {step === 1 && (
              <>
                ...
                <div style={{ display: 'flex', flexWrap: 'wrap', gap: '16px' }}>
                  ...
                  <Field label="Téléphone" name="phone" value={form.phone} onChange={update} type="tel" placeholder="+212 6XX XXX XXX" required />
                  <SelectField label="Ville" name="city" value={form.city} onChange={update} required options={[
            ...
            <div style={{ marginTop: '24px', display: 'flex', flexDirection: 'column', gap: '10px' }}>
              {['Paiement 100% sécurisé', 'Livraison rapide 2-5 jours', 'Retours gratuits 30 jours', 'Formules 100% naturelles'].map((text) => (
                <div key={text} style={{ display: 'flex', alignItems: 'center', gap: '10px', fontFamily: "'Jost', sans-serif", fontSize: '0.78rem', color: '#8C7B6A' }}>
                  <span style={{ width: '6px', height: '6px', borderRadius: '50%', background: '#6B4F32', display: 'inline-block' }} />{text}
                </div>
              ))}
            </div>
          </div>
